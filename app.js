 ...
 * 相依：必須先載入 db_spots.js 與 db_lang.js
 */
...
// 🌟 優化一：報幕即時性，改為 moveend + 600ms 黃金延遲
...
function addMarkerToMap(s) {
    ...
    if (!s.tags.includes('自訂') && !s.wikiImg && queryTitle !== "") { fetch(`https://zh.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(queryTitle)}`).then(r=>{if(!r.ok) throw new Error(); return r.json()}).then(d=>{s.wikiImg=d.thumbnail?.source;}).catch(()=>{}); }
...
window.mapInstance.on('contextmenu', function(e) {
    ...
    const tempPopup = L.popup({ closeButton: false, autoClose: false, offset: [0, -10] })
        .setLatLng(e.latlng)
        .setContent("<div style='padding:8px; font-weight:bold; color:var(--primary); font-size:13px;'><i class='fas fa-spinner fa-spin'></i> 獲取詳細地址中...</div>")
    ...
    .then(data => {
        ...
        if(data && data.address) {
            const a = data.address;
            addr = (a.city || a.county || "") + (a.town || a.suburb || a.district || "") + (a.village || "") + (a.road || "") + (a.house_number || "");
        }
        if(!addr || addr === "未知地址") addr = "瑞芳秘境";

        window.mapInstance.closePopup(tempPopup); 
    ...
    .catch(() => {
        window.mapInstance.closePopup(tempPopup);
        setTimeout(() => {
            const spotName = prompt("📍 無法獲取詳細地址\n是否仍要新增自訂景點？請為地點命名：", "我的景點");
            if (spotName) { 
                const newSpot = { name: spotName, lat: lat, lng: lng, tags: ["自訂"], highlights: "點擊下方編輯...", food: "--", history: "自訂標記", transport: "自行前往", wikiImg: "" };
                savedCustomSpots.push(newSpot); localStorage.setItem('ruifang_custom_spots', JSON.stringify(savedCustomSpots)); addMarkerToMap(newSpot); showCard(newSpot); 
            }
        }, 150);
    });
