...
// 🌟 修正：改成 dragend，只有使用者真的手滑完地圖，才要求地址，解決 425 限制
let geocodeTimer = null;
window.mapInstance.on('dragstart', () => { document.getElementById("addr-text").style.opacity = '0.5'; });
window.mapInstance.on('dragend', function() {
    ...
    geocodeTimer = setTimeout(() => {
        ...
        .then(data => { if (data && data.address) { const a = data.address; document.getElementById("addr-text").innerText = ((a.road||"") + (a.village||a.suburb||a.district||"")) || "探索瑞芳中..."; } })
...
function addMarkerToMap(s) {
    // 🌟 修正：只有在 wikiTitle 不為空的時候，才去打維基百科 API
    const queryTitle = s.wikiTitle !== undefined ? s.wikiTitle : s.name;
    if (!s.tags.includes('自訂') && !s.wikiImg && queryTitle !== "") {
        fetch(`https://zh.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(queryTitle)}`).then(r=>{if(!r.ok) throw new Error(); return r.json()}).then(d=>{s.wikiImg=d.thumbnail?.source;}).catch(()=>{});
    }
...
window.mapInstance.on('contextmenu', function(e) { const spotName = prompt("📍 新增自訂標記\n請為地點命名：", "我的景點"); if (!spotName) return; const newSpot = { name: spotName, lat: e.latlng.lat, lng: e.latlng.lng, tags: ["自訂"], highlights: "點擊下方編輯...", food: "--", history: "自訂標記", transport: "自行前往", wikiImg: "" }; savedCustomSpots.push(newSpot); localStorage.setItem('ruifang_custom_spots', JSON.stringify(savedCustomSpots)); addMarkerToMap(newSpot); showCard(newSpot); });
...
function saveEditSpot() { const newName = document.getElementById('edit-name').value.trim(); if(!newName) return alert("名稱不能為空！"); const savedIdx = savedCustomSpots.findIndex(x => x.name === currentEditingSpotName); if(savedIdx === -1) return; const s = savedCustomSpots[savedIdx]; s.name = newName; s.highlights = document.getElementById('edit-highlights').value; s.history = document.getElementById('edit-history').value; s.wikiImg = document.getElementById('edit-image-preview').src; localStorage.setItem('ruifang_custom_spots', JSON.stringify(savedCustomSpots)); if(s.markerObj) cluster.removeLayer(s.markerObj); addMarkerToMap(s); closeEditModal(); showCard(s); }
...
function renderCardButtons(s, t = translations[currentLang]) { const btnGroup = document.getElementById("card-btn-group"); if (s.tags.includes('自訂')) { btnGroup.innerHTML = `<button onclick="startNav()" style="flex: 1.2;"><i class="fas fa-location-arrow"></i> ${t.nav}</button><button class="edit-btn" onclick="openEditModal('${s.name}')"><i class="fas fa-edit"></i></button><button class="danger" onclick="deleteCustomSpot('${s.name}')"><i class="fas fa-trash-alt"></i></button>`; } else { btnGroup.innerHTML = `<button onclick="startNav()"><i class="fas fa-location-arrow"></i> ${t.nav}</button><button class="secondary" onclick="aiTrip()"><i class="fas fa-magic"></i> ${t.ai}</button>`; } }
